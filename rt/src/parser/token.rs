...
use super::{Stream, Span, Result, ErrorKind};
use crate::value::ty::{Integer, Float, Text};
use crate::value::Gc;
...
impl<'a> Token<'a> {
	pub fn parse(stream: &mut Stream<'a>) -> Result<'a, Self> {
		match stream.peek().expect("we already checked for eof") {
			c if c.is_whitespace() || c == '#' => unreachable!("we already stripped whitespace & comments"),
			...
			a if is_able_to_compose_an_operator(a) => Ok(Self::Symbol(stream.take_while(is_able_to_compose_an_operator))),
...
impl<'a> SpannedToken<'a> {
	pub fn parse(stream: &mut Stream<'a>) -> Result<'a, Option<Self>> {
		...
		Ok(Some(Self { token, span: start.finish(stream) }))
	}
}
