...
impl<'a> PatternSequence<'a> {
	fn parse(parser: &mut Parser<'a>, end: ParenType) -> Result<'a, Option<Self>> {
		...
		while !matches!(
			...
			Some(Token { contents: TokenContents::SyntaxOr(0), .. })
		) {
...
impl<'a> PatternKind<'a> {
	fn parse(parser: &mut Parser<'a>) -> Result<'a, Option<Self>> {
		match parser.take_bypass_syntax()? {
			Some(Token { contents: TokenContents::Identifier(name), .. }) => {
...
impl<'a> PatternAtom<'a> {
	fn attempt_to_parse(
		...
		match parser.take_bypass_syntax()? {
			...
			Some(Token { contents: TokenContents::SyntaxNot(0), .. }) => {
			...
			Some(
				token @ Token { contents: TokenContents::SyntaxIdentifier(..), .. }
			...
			Some(Token { contents: TokenContents::EscapedLeftParen(paren), span }) => {
				seq.push(Self::Token(Token { contents: TokenContents::LeftParen(paren), span }));
				Ok(true)
			}

			Some(Token { contents: TokenContents::EscapedRightParen(paren), span }) => {
...
impl<'a> PatternAtom<'a> {
	fn does_match(
		...
		match self {
			...
			Self::Paren(ParenType::Square, body) => {
				does_match_paren(0, Some(1), body, matcher, parser)
			}
